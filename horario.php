...
$consulta_usuario = mysql_query("SELECT `tu_Hora` ,  `tu_Fecha`, `us_Nombre`, `us_Apellido_Pat`, `us_Apellido_Mat`
FROM  `turno`
INNER JOIN usuario ON tu_Rut_Us = us_Rut
WHERE tu_Rut_Us =  '".$_SESSION['us_rut']."'
AND  `tu_Fecha` >= CURDATE( )
ORDER BY  `turno`.`tu_Fecha` ASC
LIMIT 0 , 30",$con);
$consulta_nombre=mysql_query("SELECT `us_Nombre`, `us_Apellido_Pat`, `us_Apellido_Mat` FROM usuario
								Where us_Rut='".$_SESSION['us_rut']."'",$con);
$respuesta_nombre=mysql_fetch_array($consulta_nombre);
?>
<!DOCTYPE html>
<html lang="en">
<head>
		...
		<div class="row">
			<div class="span3">
				...
				<div class="span6">
					...
					<BR>
						<h4><?php echo $respuesta_nombre['us_Nombre']." ".$respuesta_nombre['us_Apellido_Pat']." ".$respuesta_nombre['us_Apellido_Mat']?></h4>
						<?echo mysql_fechaentera(date("Y-m-d"))  ?>
						<br>
						<br>
					...
					<th>Hora</th>
					<?php $num=0; ?>
					<?php while($respuesta_usuario=mysql_fetch_array($consulta_usuario)): ?>
					<tr>
						<td><?php echo $num=$num+1; ?> </td>
						<td><?php echo $respuesta_usuario['tu_Fecha']; ?></td>
						<td>
							<?php
							switch ($respuesta_usuario['tu_Hora']) {
								case '1':
									echo "Desde las 09:00 am hasta las 12:30 pm";
									break;
								case '2':
									echo "Desde las 12:30 pm hasta las 16:00 pm";
									break;
								case '3':
									echo "Desde las 16:00 pm hasta las 19:30 pm";
									break;
								case '4':
									echo "Desde las 19:30 pm hasta las 22:00 pm";
									break;
							}
							?>
						</td>
