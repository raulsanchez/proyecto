...
if(isset($_SESSION['us_rut'])):
	$consulta_msj = mysql_query("SELECT co_mensaje, co_asunto, `mu_id_msj`
								FROM `mensaje_usuario`
								INNER JOIN mensaje ON mu_id_msj = co_id_mensaje
								INNER JOIN usuario ON mu_Rut_Us = us_Rut
								WHERE mu_Rut_Us = '".$_SESSION['us_rut']."'",$con);
else:
	$consulta_msj = mysql_query("SELECT `co_mensaje`, `co_asunto` FROM `mensaje`",$con);
endif;
		...
		<div class="row">
			<div class="span12">
				<table class="table table-striped table-hover table-bordered table-condensed ">
					...
					<th>Mensaje</th>
					<?php $num=0; ?>
					<?php while($respuesta_msj=mysql_fetch_array($consulta_msj)): ?>
					<tr>
						<td><?php echo $num=$num+1; ?> </td>
						<td><?php echo $respuesta_msj['co_asunto']?></td>
						<td><?php echo $respuesta_msj['co_mensaje']?></td>
						<td>
							<a class="btn btn-small" href="ver_mensaje.php?ver=<?php echo $respuesta_msj["mu_id_msj"]; ?>">
										<i class="icon-envelope"></i> Ver </a>
						</td>
					</tr>
					<?php endwhile; ?>
				</table>
